...
SELECT id,
        ...
        (SELECT 
			hometown
			FROM profiles
			WHERE users.id = user_id) AS "City",
		(SELECT 
			filename
			FROM media
			WHERE  
				(SELECT
					photo_id
                    FROM profiles
                    WHERE users.id = user_id) = media.id) AS "Avatar"
FROM users;
...
-- (используя UNION)
SELECT target_user_id as result
FROM friend_requests 
WHERE friend_requests.status = "approved" AND initiator_user_id = 1
UNION
SELECT initiator_user_id
FROM friend_requests 
WHERE friend_requests.status = "approved" AND target_user_id = 1
ORDER BY result;

-- выбрать всех пользователей, указав их id, имя и фамилию, город и аватарку
-- (используя JOIN)
SELECT users.id, firstName, LastName, hometown, filename
FROM users
LEFT JOIN profiles
...
WHERE media_types.name_type = @type;
